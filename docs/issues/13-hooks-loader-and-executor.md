...
1. Types: implement against the structural interfaces `HookContext` / `HooksRegistry` declared
   in `src/core/types.ts` by issue 11 — do not change their shapes. This module exports exactly:
   ```ts
...
2. `loadHooks`:
   ...
   - `loadHooks` performs one dynamic import per call and no caching of its own; calling it
     once per run and passing the registry down is the runner's job (issue 18).
...
4. `hook` step in `steps.ts`: `ctx.hooks === null` → `HOOK_FAILED` with hint
   ...
   `hooks.invoke(step.hook, hookCtx, timeoutMs)` where
   `timeoutMs = step.timeoutMs ?? ctx.defaultTimeoutMs` (step shapes: shorthand `hook: "name"`
   or `{ hook: "name", timeoutMs? }`, issue 04), wrapped with the standard step attribution
...
- [ ] A synchronous exported function (`export function syncHook(ctx) { … }`) runs successfully —
      its return value is awaited (DESIGN §8 allows sync or async).
