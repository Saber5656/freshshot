...
3. Human output: stale shots list their `changedRatio` and diff artifact path; a final hint line
   `run 'freshshot update' to refresh N stale screenshot(s)` when exit code is 1 (only for
   stale/missing-baseline, not for failed).
4. Guarantee: no file under the project root outside `.freshshot/` is created, modified, or
